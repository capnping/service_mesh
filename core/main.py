...
def add_book():
    ...
    try:
        ...
        db.session.add(book)
        db.session.commit()
        update_price_data = {'book_name': content['name'], 'book_price': round(random.uniform(20.00, 30.00), 2)}
        print(update_price_data)
        requests.post(url='http://localhost:5005/book/add_book/', json=update_price_data)
